package org.hil.core.util
{
	...
	public class StringUtils
	{
		 ...
		 *	@tiptext
		 */
		public static function capitalize(p_string:String, p_all:Boolean):String
		{
			...
			if (p_all === true)
			{
				...
				/*
				   return str.replace(/\b./g, function(... m):String
				   {
...
//			return str.replace(/\b./g,function(...m):String{return m[0].toUpperCase()});
		}
		...
		// added by Le Anh Tuan

		/**
		 *	Fill from the left of the p_string with p_char, the number of p_char = p_length - p_char.length
		 *
		 *	@param p_string The string.
		 *  @param p_char The char to fill.
		 *  @param p_length: maximum number of p_char
		 *	@returns String
		 *
		 * 	@langversion ActionScript 3.0
		 *	@playerversion Flash 9.0
		...
		public static function fillLeft(p_string:String, p_char:String, p_length:Number):String
		{
			var p_new_string:String=p_string;



			if (p_string.length != 0 && p_string.length < p_length)
			{
				for (var i:Number=0; i < p_length - p_string.length; i++)
				{
					p_new_string=p_char + p_new_string;
				}
			}
			return p_new_string;
		}
