package org.hil.core.components
{
	...
	public class TextInputUTF8 extends mx.controls.TextInput
	{
		public var enableValidator:Boolean = false;
		public var minTextValidatorLength:Number = -1;
		public var maxTextValidatorLength:Number = -1;
		public var required:Boolean = false;
		
		public function TextInputUTF8()
		{
			super();
		}
		...
		override public function addEventListener(
            ...
            if (type == Event.CHANGE && !hasEventListener(type))
            {
            	...
            	if(enableValidator)
	        	{
	        		super.addEventListener(type, validTextInput, useCapture, priority, useWeakReference);
	        	}
            }
        ...
        // valid text input
		public function validTextInput(event:Event):void
		{
			//Alert.show("test");
			var textValidator:StringValidator = new StringValidator();
			textValidator.source = event.currentTarget;
			textValidator.property = "text";
			
			// valid required
			if(required)
			{
				textValidator.required = true;
				textValidator.requiredFieldError = resourceManager.getString('Validator','loi_bo_trong');
			}
			else{
				textValidator.required = false;
			}
			
			// valid min length
			if(minTextValidatorLength != -1)
			{
				textValidator.minLength = minTextValidatorLength;
				textValidator.tooShortError = resourceManager.getString('Validator','loi_nho_hon') + " " + textValidator.minLength;
			}
			
			// valid max length
			if(maxTextValidatorLength != -1)
			{
				textValidator.maxLength = maxTextValidatorLength;
				textValidator.tooLongError = resourceManager.getString('Validator','loi_lon_hon') + " " + textValidator.maxLength;
			}	
		}
